...
If you've ever worked on a sizeable Rails project, you've probably experienced some codebase quirks at some point.  For example, I've got an application that had issues when the "Liquid Templating engine":http://liquidmarkup.org/ was loaded as a gem.  The fix was to include a specific version as a plugin.  While it's usually best to get to the root of the problem, sometimes it's not feasible or expedient and such measures must suffice.  Once Liquid was included as a gem, we noticed some failing tests, and then pieced together that, oops, Liquid had be reconfigured as a gem.  A code review may have caught this, or perhaps not, since it's a little quirky.  Now the second time we made the mistake, it dawned on me, write a spec to send up a red flag and make it easier to spot the immediate problem when tests fail in the future:
